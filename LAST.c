...
int faireDesTours( int i ) {
    ...
    while (shared_memory[i].tempsTotal <= tempsMaxCircuit && !shared_memory[i].isOut) //time pas dépassée
    {
        ...
        if (shared_memory[i].s1 <= theBest.best_S1) {
            theBest.best_S1 = shared_memory[i].s1;
        }
        ...
        if (shared_memory[i].s2 <= theBest.best_S2) {
            theBest.best_S2 = shared_memory[i].s2;
        }
        ...
        if (shared_memory[i].s3 <= theBest.best_S3) {
            theBest.best_S3 = shared_memory[i].s3;
        }
        ...
        if (tour_complet < shared_memory[i].best_Circuit) {
            ...
            theBest.best_Circuit = tour_complet;
        }
        /* *************************************** */
    }
...
void initVoiture(int i) {
    ...
    shared_memory[i].tempsTotalComplet = 0;
...
void afficherTableau() {
    ...
    //printf("bs1: %d, bs2: %d, bs3: %d et b_circuit %d", theBest.best_S1, theBest.best_S2, theBest.best_S3, theBest.best_Circuit );
...
void reloadVoiture( int i , int s1 , int s2 , int s3 , int bestS1 , int bestS2 , int bestS3 , int bestCircuit , int lap , int compteurStand) {
    ...
    // pas bon !!
    theBest.best_S1 += bestS1;
    theBest.best_S2 += bestS2;
    theBest.best_S3 += bestS3;
